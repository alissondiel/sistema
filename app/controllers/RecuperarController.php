  ...
  public function solicitarAction()
  {
    ...
    if(!is_null($email) && $email !== false){
      ...
      if($validar->status === false){
        ...
        $this->load( //LOAD CARREGA TOKEN NO FINAL DA URL
            'Services\PasswordRecovery',
            $this->configs->site->url . $this->configs->baseURI . 'recuperar/redefinir/' //CHAMA METODO REDEFINIRACTION()
          );
          ...
          $this->load('Services\Email');
            ...
            $envioDoEmail = $this->email->send(
                $validar->user->email,
                'DIVVINA BOUTIQUE - ' . $message['subject'],
                $message['message'] . 'DIVVINA BOUTIQUE',
                array(
